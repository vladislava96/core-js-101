...
function getRectangleString(width, height) {
  function drawLine(first, fill, last) {
    let str = '';
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const inputLetter = str[i];
    const index = input.indexOf(inputLetter);
    const outputLetter = output[index];
    result += outputLetter;
  }
  return result;
}
...
function getCardId(value) {
  ...
  const index = cards.findIndex((elem) => elem === value);
